...
# 外部変数
#================================================================
read_index = ctypes.c_ulong(0)                  # リードポインタ
write_index = ctypes.c_ulong(0)                 # ライトポインタ
pre_count = ctypes.c_ulong(0)                   # 前回カウント値
pre_carry  = ctypes.c_ulong(0)                  # 前回キャリー値
file_name = 'infinite.txt'                      # 保存ファイル名
...
main_scr = {
    ...
    # メニュー[階層][メニュー個数]
    #----------------------------------------
    'menu':[['DioInit','Condition...','Start','Stop','Count','Status','DioExit','Exit'],[]],
    #----------------------------------------
    # 上部ステータスエリアデータ
    #----------------------------------------
    'status_top':[],
...
set_scr = {
    ...
    # メニュー[階層][メニュー個数]
    #----------------------------------------
    'menu':[['Start','Clock','Stop','OK','Cancel'],[]],
    #----------------------------------------
    # 項目選択タイプ用設定データ
    #----------------------------------------
    'select_item':[{'pos_y':0,                          # 設定値表示画面で何段目に表示するか？
                    ...
                    {'pos_y':4,
                    'set_item_name':'Notification',
                    'item':[{'name':'Half Full','num':0}],
                    'set_num':0
                    }],
    #----------------------------------------
    # 数値入力タイプ用設定データ
...
def count_callback(dev_id, message, dir, param):
    ...
    if message == cdio.DIO_DMM_COUNT:
        ...
        # ライトポインタ取得
        #----------------------------------------
        lret = cdio.DioDmGetWritePointerUserBuf(dev_id, cdio.DIODM_DIR_IN, ctypes.byref(write_index), ctypes.byref(count), ctypes.byref(carry))
        if lret != TermLib.TERM_ERR_SUCCESS:
            cdio.DioGetErrorString(lret, err_str)
            buf = str.format('DioDmGetWritePointerUserBuf {}:{}', lret, err_str.value.decode('sjis'))
            TermLib.TermSetStatus(TermLib.STATUS_AREA_BOTTOM, STS_RET_POS, buf, False)
            return
        #----------------------------------------
        # オーバーランエラーチェック
        #----------------------------------------
        if carry.value == pre_carry.value:
            if count.value > (pre_count.value + DATA_SIZE):
                buf = str.format('Overrun Error! (Count = {})', count.value)
        ...
        try:
            ...
            buf = str.format('fopen : file open error.')
            TermLib.TermSetStatus(TermLib.STATUS_AREA_BOTTOM, STS_RET_POS, buf)
            return
        #----------------------------------------
        # データ書き込み
        #----------------------------------------
        if write_index.value > read_index.value:
            cnt = count.value - (write_index.value - read_index.value)
            for i in range(read_index.value, write_index.value):
                buf = str.format('{0:8d} : {1:08X}\n', cnt, data_buff[i])
                the_file.write(buf)
                cnt += 1
        else:
            cnt = count.value - (DATA_SIZE - read_index.value + write_index.value)
            for i in range(read_index.value, DATA_SIZE):
                buf = str.format('{0:8d} : {1:08X}\n', cnt, data_buff[i])
                the_file.write(buf)
                cnt += 1
            for i in range(write_index.value):
                buf = str.format('{0:8d} : {1:08X}\n', cnt, data_buff[i])
                the_file.write(buf)
                cnt += 1
        #----------------------------------------
    ...
    # リードポインタを設定
    #----------------------------------------
    read_index.value = write_index.value
    pre_count.value = count.value
    pre_carry.value = carry.value
    #----------------------------------------
    # カウント値を表示
    #----------------------------------------
    buf = str.format('{:<30d}', count.value)
    TermLib.TermSetStatus(TermLib.STATUS_AREA_BOTTOM, STS_COUNT_POS, buf, False)
    return
...
def condition(dev_id, err_str):
    menu_num = -1
    start = clock = stop = -1
    comp_ptn = ptn_mask = internal_clock = stop_num = -1
    ...
    while True:
        ...
        # メニュー番号入力
        #----------------------------------------
        lret, menu_num = TermLib.TermGetMenuNum('Please input a menu number(0-4).', menu_num)
        if lret != TermLib.TERM_ERR_SUCCESS:
        ...
        if menu_num == SET_MENU_START:
            ...
            if start == cdio.DIODM_START_PATTERN:
                lret, comp_ptn = TermLib.TermGetInputNum(set_scr, SET_NUM_COMP_PTN, comp_ptn)
                if lret != TermLib.TERM_ERR_SUCCESS:
                    err_str = 'TermGetInputNum ' + str(lret)
                lret, ptn_mask = TermLib.TermGetInputNum(set_scr, SET_NUM_PTN_MASK, ptn_mask)
                if lret != TermLib.TERM_ERR_SUCCESS:
                    err_str = 'TermGetInputNum ' + str(lret)
        #----------------------------------------
        # クロック条件設定
        ...
        #----------------------------------------
            ...
            # Direction
            #----------------------------------------
            lret = cdio.DioDmSetDirection(dev_id, cdio.PI_32)
            ...
            # Pattern/Mask
            #----------------------------------------
            lret = cdio.DioDmSetStartPattern(dev_id, comp_ptn, ptn_mask)
            if lret != cdio.DIO_ERR_SUCCESS:
                cdio.DioGetErrorString(lret, get_str)
...
def main():
    ...
    #----------------------------------------
    while True:
        lret, menu_num = TermLib.TermGetMenuNum('Please input a menu number(0-7)', menu_num)
        if lret != TermLib.TERM_ERR_SUCCESS:
            print('RetCode = {}'.format(lret))
            break
        #----------------------------------------
        ...
        #----------------------------------------
            ...
            if lret != cdio.DIO_ERR_SUCCESS:
                ...
                TermLib.TermSetStatus(TermLib.STATUS_AREA_BOTTOM, STS_RET_POS, buf)
                continue
            lret = cdio.DioDmSetCountEvent(dio_id, dir, int(DATA_SIZE / 2), pcount_callback, "")
            if lret != cdio.DIO_ERR_SUCCESS:
                cdio.DioGetErrorString(lret, get_str)
            ...
            # Set Buffer
            #----------------------------------------
            lret = cdio.DioDmSetBuff(dio_id, dir, ctypes.cast(data_buff, ctypes.POINTER(ctypes.c_ulong)), DATA_SIZE, cdio.DIODM_WRITE_RING)
            if lret != cdio.DIO_ERR_SUCCESS:
                cdio.DioGetErrorString(lret, get_str)
