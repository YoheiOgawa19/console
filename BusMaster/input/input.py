...
main_scr = {
    ...
    # メニュー[階層][メニュー個数]
    #----------------------------------------
    'menu':[['DioInit','Condition...','Start','Stop','Data','Count','Status','DioExit','Exit'],[]],
    #----------------------------------------
    # 上部ステータスエリアデータ
    #----------------------------------------
    'status_top':[],
...
set_scr = {
    ...
    # メニュー[階層][メニュー個数]
    #----------------------------------------
    'menu':[['Start','Clock','Stop','Notification','OK','Cancel'],[]],
    #----------------------------------------
    # 項目選択タイプ用設定データ
    #----------------------------------------
...
def count_callback(dev_id, message, dir, param):
    ...
    if message == cdio.DIO_DMM_COUNT:
        ...
        if lret == cdio.DIO_ERR_SUCCESS:
            TermLib.TermSetStatus(TermLib.STATUS_AREA_BOTTOM, STS_COUNT_POS, str(count.value), False)
        else:
...
def condition(dev_id, err_str):
    ...
    while True:
        ...
        # メニュー番号入力
        #----------------------------------------
        lret, menu_num = TermLib.TermGetMenuNum('Please input a menu number(0-5).', menu_num)
        if lret != TermLib.TERM_ERR_SUCCESS:
...
def main():
    ...
    while True:
        ...
        #----------------------------------------
            ...
            # Set Buffer
            #----------------------------------------
            lret = cdio.DioDmSetBuff(dio_id, dir, ctypes.cast(data_buff, ctypes.POINTER(ctypes.c_ulong)), DATA_SIZE, cdio.DIODM_WRITE_ONCE)
            if lret != cdio.DIO_ERR_SUCCESS:
                cdio.DioGetErrorString(lret, get_str)
        ...
        #----------------------------------------
            ...
            #----------------------------------------
            the_file.close()
            buf = str.format('{} にデータを保存しました。', file_name)
            TermLib.TermSetStatus(TermLib.STATUS_AREA_BOTTOM, STS_RET_POS, buf)
        #----------------------------------------
        # 転送カウント取得
        #----------------------------------------
